...
let scroll;  // Single declaration of scroll variable
...
function initPageTransitions() {
  ...
  barba.hooks.after(() => {
    ...
    // reinit locomotive scroll with a slight delay
    setTimeout(() => {
      scroll.init();
      scroll.update();
...
function initAjaxContactForm() {
  ...
  form.addEventListener('submit', async function(e) {
    ...
    const existingErrors = form.querySelectorAll('.alert.alert-error');
    ...
    if (!nameField.value.trim()) {
      ...
      nameError.className = 'alert alert-error';
      nameError.innerHTML = '<span>Please enter a valid name</span>';
      nameField.parentElement.appendChild(nameError);
      window.scrollTo({ top: nameField.offsetTop, behavior: 'smooth' });
    }
    ...
    if (!emailField.value.trim() || !emailRegex.test(emailField.value)) {
      ...
      emailError.className = 'alert alert-error';
      emailError.innerHTML = '<span>Please enter a valid email address</span>';
      emailField.parentElement.appendChild(emailError);
    }

    // Validate message (required and length)
    const messageText = messageField.value.trim();
    if (!messageText || messageText.length < 3 || messageText.length > 3000) {
      isValid = false;
      const messageError = document.createElement('div');
      messageError.className = 'alert alert-error';
      messageError.innerHTML = '<span>Please enter a text between 3 and 3000 characters</span>';
      messageField.parentElement.appendChild(messageError);
    }

    // Scroll to first error if validation failed
    if (!isValid) {
      const firstError = form.querySelector('.alert.alert-error');
      if (firstError) {
        const formCol = firstError.closest('.form-col');
        if (formCol) {
          scroll.scrollTo(formCol, {
            offset: -100,
            duration: 800,
    ...
    try {
      ...
      if (response.ok) {
        // Use Barba.js to navigate to success page
        barba.go('/success.html');
      } else {
        // Use Barba.js to navigate to error page
        barba.go('/error.html');
      }
    } catch (error) {
      // Use Barba.js to navigate to error page
